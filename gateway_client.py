...
import http.client, json, socket, urllib.error, urllib.parse, urllib.request

class GatewayError(RuntimeError):
    def __init__(self,kind,message,status=None):super().__init__(message);self.kind=kind;self.status=status

class GatewayClient:
    SERVER_HINT='PLC Gateway is unavailable. Verify Uvicorn is running on BBCMWPBMX13, then reconnect.'
    def __init__(self,base_url,timeout=8.0,authenticated_user=None):self.base_url=base_url.rstrip('/');self.timeout=timeout;self.authenticated_user=authenticated_user
    def _request(self,method,path,params=None,body=None):
        ...
        try:
            ...
            code=detail.get('code','http_error') if isinstance(detail,dict) else 'http_error';msg=detail.get('message',str(detail)) if isinstance(detail,dict) else str(detail);raise GatewayError(code,msg,exc.code) from exc
        except (urllib.error.URLError,http.client.RemoteDisconnected,ConnectionResetError,ConnectionAbortedError,BrokenPipeError,socket.timeout,TimeoutError,OSError) as exc:
            detail=getattr(exc,'reason',exc);raise GatewayError('gateway_unavailable','%s (%s)'%(self.SERVER_HINT,detail)) from exc
        except (json.JSONDecodeError,UnicodeDecodeError) as exc:raise GatewayError('gateway_invalid_response','PLC Gateway returned an invalid response. Verify Uvicorn is healthy on BBCMWPBMX13, then reconnect.') from exc
    ...
    def schema_type(self,plc,type_name):return self._request('GET','/schema/type/'+urllib.parse.quote(type_name,safe=''),{'plc':plc})
