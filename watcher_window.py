...
class WatcherWindow:
    ...
    def __init__(self, parent, sessions, schema_loader):
        ...
        self.items = {}              # (plc,path) -> tree iid
        self.nodes = {}              # iid -> (plc,node)
        self.schemas = {}            # plc -> Schema
        self.loaded = set()
        self.results = queue.Queue()
        self.busy = set()            # PLC names with a read in flight
        self.closed = False
        self.read_enabled = tk.BooleanVar(master=parent, value=True)

        self.top = tk.Toplevel(parent)
        self.top.title('Watcher')
        self.top.geometry('1180x700')
        self.top.protocol('WM_DELETE_WINDOW', self.hide)

        bar = ttk.Frame(self.top, padding=6); bar.pack(fill='x')
        ttk.Checkbutton(bar, text='Live Read', variable=self.read_enabled).pack(side='left')
        ttk.Button(bar, text='Remove selected', command=self.remove_selected).pack(side='left', padx=6)
        ttk.Button(bar, text='Clear Watcher', command=self.clear).pack(side='left')
        self.status = tk.StringVar(value='Watcher is read-only. Add leaves or branches from Expander.')
        ttk.Label(bar, textvariable=self.status).pack(side='left', padx=12)

        frame = ttk.Frame(self.top); frame.pack(fill='both', expand=True, padx=6, pady=6)
        self.tree = ttk.Treeview(frame, columns=('plc','value','type','state'), show='tree headings')
        self.tree.heading('#0', text='Tag / member'); self.tree.column('#0', width=520)
        for key,title,width in [('plc','PLC',110),('value','Value',180),('type','Logix type',150),('state','Status',180)]:
    ...
    def add(self, plc, node):
        ...
        if key in self.items:
            self.tree.selection_set(self.items[key]); self.tree.see(self.items[key]); self.show(); return
        iid=self._insert('',plc,node,root=True); self.items[key]=iid; self.show()
        self.status.set('Added %s::%s' % key)

    def _insert(self,parent,plc,node,root=False):
        label=node.path if root else node.name
        dtype=node.dtype+(str(node.dimensions) if node.dimensions else '')
        iid=self.tree.insert(parent,'end',text=label,values=(plc,'',dtype,'Not reading' if node.selectable else 'Definition'))
        self.nodes[iid]=(plc,node)
    ...
    def _schema(self,plc):
        if plc not in self.schemas:
            ...
            schema,_source=self.schema_loader(session,False)
            self.schemas[plc]=schema
    ...
    def remove_selected(self):
        ...
        for iid in selection:
            self._forget_subtree(iid); self.tree.delete(iid)
        self.status.set('Removed selected Watcher item.')
    ...
    def _start_reads(self):
        ...
        for plc,leaves in self.visible_leaves().items():
            ...
            batch=leaves[:self.BATCH_SIZE]; session=self.sessions.get(plc)
            if session is None:continue
            self.busy.add(plc)
            def worker(plc=plc,batch=batch,session=session):
                rows=[]
                try:
                    if not session.connected:
                        ok,msg=session.connect()
    ...
    def pump(self):
        ...
        try:
            while True:
                plc,rows=self.results.get_nowait(); self.busy.discard(plc)
                for iid,path,value,status in rows:
                    pair=self.nodes.get(iid)
                    if pair and pair[0]==plc and pair[1].path==path:
                        self.tree.set(iid,'value',value);self.tree.set(iid,'state',status)
