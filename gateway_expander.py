...
@dataclass
class Node:
    name: str; path: str; datatype: str = "Unknown"; dimensions: tuple = ()
    access: str = "Unknown"; structured: bool = False; expandable: bool = False; kind: str = "member"
    @property
    def selectable(self): return not self.expandable and not self.dimensions and self.access != "None"

def node_from(raw):
    dims=tuple(raw.get('dimensions') or ())
    structured=bool(raw.get('structured'))
    # /schema roots predate the GUI-ready expandable flag; derive it from metadata too.
    expandable=bool(raw.get('expandable')) or structured or bool(dims)
    return Node(raw.get('name',raw.get('path','')),raw.get('path',''),raw.get('datatype','Unknown'),dims,
                raw.get('access','Unknown'),structured,expandable,raw.get('kind','member'))

class GatewayExpander:
    PAGE=100; READ_INTERVAL_MS=750
    def __init__(self,root,client,initial_plc=None):
        ...
        root.title('PLC Gateway Expander - read-only validation'); root.geometry('1180x760')
        ...
        bar=ttk.Frame(root,padding=6); bar.pack(fill='x'); ttk.Label(bar,text='PLC:').pack(side='left')
        box=ttk.Combobox(bar,textvariable=self.plc,values=plcs,state='readonly',width=18); box.pack(side='left',padx=6); box.bind('<<ComboboxSelected>>',lambda _e:self.load_roots())
        ttk.Button(bar,text='Load tags',command=self.load_roots).pack(side='left'); ttk.Checkbutton(bar,text='Read visible leaves',variable=self.live,command=self.toggle_live).pack(side='left',padx=12)
        ttk.Label(root,textvariable=self.status,anchor='w').pack(fill='x',padx=6)
        search=ttk.Frame(root,padding=(6,2)); search.pack(fill='x'); ttk.Label(search,text='Root filter:').pack(side='left'); ent=ttk.Entry(search,textvariable=self.filter); ent.pack(side='left',fill='x',expand=True,padx=6); ent.bind('<Return>',lambda _e:self.load_roots()); ttk.Button(search,text='Search',command=self.load_roots).pack(side='left')
        ...
        actions=ttk.Frame(root,padding=6); actions.pack(fill='x'); ttk.Button(actions,text='Read selected once',command=self.read_selected).pack(side='left'); ttk.Button(actions,text='Go to array index...',command=self.goto_index).pack(side='left',padx=6); ttk.Button(actions,text='Copy selected path',command=self.copy_selected).pack(side='left'); ttk.Label(actions,text='Read-only gateway client.').pack(side='left',padx=12)
    ...
    def expand(self,_event=None):
        ...
        self.tree.insert(iid,'end',text='Loading...'); plc=self.plc.get(); self.status.set('Expanding '+node.path+' through gateway...'); self._worker(lambda:self.client.expand(plc,node.path,limit=self.PAGE),lambda p:self._expanded(iid,node,p))
    def _expanded(self,iid,node,payload):
        for child in self.tree.get_children(iid): self.tree.delete(child)
        for raw in payload.get('children',[]): self.insert(iid,node_from(raw))
        total=int(payload.get('total',0)); shown=len(payload.get('children',[])); self.loaded.add(iid); self.status.set(f"Expanded {node.path}. Showing {shown} of {total}." if total>shown else f"Expanded {node.path}.")
    def selected(self):
        sel=self.tree.selection(); return (sel[0],self.nodes.get(sel[0])) if sel else (None,None)
    def goto_index(self):
        iid,node=self.selected()
        if not node or not node.dimensions:
            messagebox.showinfo('Select an array','Select an array tag/member, then choose Go to array index.',parent=self.root); return
        total=int(node.dimensions[0]); index=simpledialog.askinteger('Array index',f'Enter index 0 through {total-1}:',parent=self.root,minvalue=0,maxvalue=total-1)
        if index is None:return
        for child in self.tree.get_children(iid): self.tree.delete(child)
        self.loaded.discard(iid); plc=self.plc.get(); offset=(index//self.PAGE)*self.PAGE; self.status.set(f'Loading {node.path} around index {index}...')
        self._worker(lambda:self.client.expand(plc,node.path,limit=self.PAGE,offset=offset),lambda p:self._indexed(iid,node,index,p))
    def _indexed(self,iid,node,index,payload):
        for child in self.tree.get_children(iid): self.tree.delete(child)
        target=None
        for raw in payload.get('children',[]):
            child=self.insert(iid,node_from(raw));
            if raw.get('name')==f'[{index}]': target=child
        self.loaded.add(iid); self.tree.item(iid,open=True)
        if target: self.tree.selection_set(target); self.tree.focus(target); self.tree.see(target)
        self.status.set(f'Loaded {node.path}[{index}] page; select its arrow to expand the UDT.')
    ...
    def copy_selected(self):
        ...
        if node:self.root.clipboard_clear(); self.root.clipboard_append(self.plc.get()+'::'+node.path)
    ...
    def live_tick(self):
        if self.live.get() and not self.reading:
            ...
            if leaves:
                self.reading=True; plc=self.plc.get(); paths=[p for _,p in leaves]; self._worker(lambda:self.client.read_batch(plc,paths),lambda p:self._batch_ready(leaves,p))
