...
class ExpandablePLCApp(PLCApp):
    ...
    def _build_ui(self):
        ...
        self.browser=LiveWatchBrowser(self.root,self.var_browser_plc.get(),self.add_browser_leaf,online_loader=lambda force:None,embedded=True)
    ...
    def on_close(self): self._closing=True;self.browser.close();super().on_close()
    def open_tag_browser(self):
        name=self.var_taglist_plc.get();session=self.sessions.get(name);browser=WatchBrowser(self.root,name,self.add_browser_leaf,online_loader=lambda force:self.discover_session(session,force))
    ...
    def discover_session(self,session,refresh=False):
        ...
        with session.lock:
            ...
            schema,source=load_gateway_schema(session.client,session.name,refresh=refresh)
            entries=[]
    ...
    def _do_connect(self,session):
        ...
        try:
            _,source=self.discover_session(session);self._status('[%s] %s'%(session.name,source))
    ...
    def _do_load_tags(self,session):
        try:
            _,source=self.discover_session(session,refresh=True);self._status('[%s] %s'%(session.name,source))
